...
def compile_code(code_string):
    ...
    Retorna SystemExit em caso de erro.
    """
    reset_compiler()
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Uso: py compiler.py <nome_do_arquivo>.txt")
    else:
        filename = sys.argv[1]
        try:
        ...
        try:
            # Recompila para obter o RPN e a tabela de símbolos
            ...
            print("---ETAPA 4 : CÓDIGO INTERMEDIÁRIO (RPN) ---")
            for line in rpn:
                print(f" {line}")
            print("-" * 40)
