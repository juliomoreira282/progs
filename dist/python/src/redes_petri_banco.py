class CaixaEletronicoPetri():
    ...
    def depositar(self, valor = 0.0):
        ...
        print(f"Depósito de R$ {valor} realizado. Saldo atual: R$ {self.places['Saldo']:.2f} \n")
    
    def sacar(self, valor = 0.0):
        if self.places["Saldo"] >= valor:
            self.places["Saque"] += valor
            self.places["Saldo"] -= valor
            print(f"Saque de R$ {valor} realizado. Saldo atual: R$ {self.places['Saldo']:.2f} \n")
        else:
