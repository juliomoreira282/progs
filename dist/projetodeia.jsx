...
const ChatBot = () => {
    ...
    const sendMessage = (userMessage) => __awaiter(void 0, void 0, void 0, function* () {
        const newMessage = { user: 'Você', message: userMessage };
        setMessages([...messages, newMessage]);
        setInput('');
        try {
            const response = yield axios.post('/api/chatbot', { message: userMessage });
    ...
    return (<div className="chatbot-container">
            ...
            {messages.map((msg, index) => (<div key={index} className={msg.user === 'Bot' ? 'bot-message' : 'user-message'}>
                    <strong>{msg.user}</strong>{msg.message}
        </div>))}
        </div>
        <input type="text" value={input} onChange={handleInputChange} onKeyUp={handleKeyPress} placeholder="Digite sua mensagem..."/>
        </div>);
};
